...
const EMAIL = 'dthusky@gmail.com';
const AWS = require('aws-sdk');
const SNS = new AWS.SNS({apiVersion: '2010-03-31'});

/**
 * Index: entry point for the app
 *
 * @param event
 * @param context
 * @param callback
...
function findExistingSubscription(topicArn, nextToken, cb) {
	...
	SNS.listSubscriptionsByTopic(params, (err, data) => {
		...
		const subscription = data.Subscriptions.filter((sub) =>
		sub.Protocol === 'email' && sub.Endpoint === EMAIL)[0];
		if (!subscription) {
			if (!data.NextToken) {
